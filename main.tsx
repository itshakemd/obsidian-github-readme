...
export default class GitHubReadmePlugin extends Plugin {
  ...
  async openReadme(fullName: string, content: string): Promise<string> {
    ...
    if (folder) { const parts = folder.split("/"); let cur=""; for(const p of parts){ cur=cur?`${cur}/${p}`:p; if(!this.app.vault.getAbstractFileByPath(cur)){ try{ await this.app.vault.createFolder(cur);}catch{}} } }
    let file = this.app.vault.getAbstractFileByPath(path); if (file instanceof TFile) { await this.app.vault.modify(file, content); } else { file = await this.app.vault.create(path, content); }
    if (!(file instanceof TFile)) return path; let target: WorkspaceLeaf | null = null; this.app.workspace.iterateAllLeaves((leaf)=>{ if(!target && leaf.view instanceof MarkdownView && (leaf.view as any).file?.path===file.path) target=leaf; }); const leaf: WorkspaceLeaf = target ?? this.app.workspace.getLeaf("tab"); await leaf.openFile(file); await this.app.workspace.revealLeaf(leaf); return path; }
