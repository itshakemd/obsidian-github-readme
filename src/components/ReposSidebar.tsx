...
export default function ReposSidebar({ getToken, listRepos, fetchProfile, onSelectRepo }: Props) {
  ...
  return <div className="github-repos-sidebar"><div className="github-repos-sidebar-header"><h4>GitHub Repos</h4></div>{repos.map((repo) => (<button key={repo.id} className="github-repos-item" onClick={() => onSelectRepo(repo)}><span className="github-repos-name">{repo.name}</span>{repo.private && <span className="github-repos-private">private</span>}</button>))}</div>;
}
